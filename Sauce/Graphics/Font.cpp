...
void DirectWrite::Initialize()
{
	...
	// バックバッファの取得
	// 型：IDXGISwapChain
	Graphic::Instance().GetSwapChain()->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));

	// dpiの設定
	//pD2DFactory->GetDesktopDpi(&dpiX, &dpiY);
	FLOAT dip = (FLOAT)GetDpiForWindow(ghWnd);

	// レンダーターゲットの作成
	D2D1_RENDER_TARGET_PROPERTIES props =
		D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_UNKNOWN, D2D1_ALPHA_MODE_PREMULTIPLIED), dip, dip);
